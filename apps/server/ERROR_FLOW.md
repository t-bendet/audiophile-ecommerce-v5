...
└────┬────┘  └────┬────┘  └────┬─────┘  └────┬─────┘  └────┬────┘       │
     ...
     └────────────┴─────────────┴─────────────┴─────────────┴─────────────┘
          ...
          ├──────────────────────┤   ├──────────────────────┤
          │ Return:              │   │ res.err → one        │
          │ ✓ status             │   │ error-level log line │
          │ ✓ message            │   │ Return:              │
          │ ✓ statusCode         │   │ ✓ status: "error"    │
          │ ✓ requestId          │   │ ✓ message: "Something│
          │                      │   │   went very wrong!"  │
          │ ❌ NO stack trace    │   │ ✓ statusCode: 500    │
          │ ❌ NO error details  │   │ ✓ requestId          │
          │                      │   │ ❌ NO stack trace    │
          │                      │   │ ❌ NO error details  │
          └──────────────────────┘   └──────────────────────┘
```
...
Every response carries an `x-request-id` header - the id the client sent, or one generated by
`pino-http` (`utils/logger.ts`). The same id is on the error envelope as `requestId` and on the
